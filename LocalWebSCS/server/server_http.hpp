...
namespace SimpleWeb {
    ...
    public:
        ...
        typedef std::map<std::string, std::unordered_map<std::string, 
			std::function<void(std::ostream&, std::shared_ptr<Request> request)> > > resource_type;
		...
		void setSignal(){
			boost::asio::signal_set signals_(m_io_service, SIGINT);
			signals_.async_wait(
    ...
    protected:
		...
		ServerBase(const std::string& ip, unsigned short port, size_t num_threads, size_t timeout_request, size_t timeout_send_or_receive) :
			endpoint(boost::asio::ip::address::from_string(ip), port), acceptor(m_io_service, endpoint), num_threads(num_threads),
			timeout_request(timeout_request), timeout_content(timeout_send_or_receive)
		{
    ...
    public:
		...
		Server(const std::string& ip, unsigned short port, size_t num_threads = 1, size_t timeout_request = 5, size_t timeout_content = 300) :
			ServerBase<HTTP>::ServerBase(ip,port, num_threads, timeout_request, timeout_content) {};
    private:
        void accept() {
